...
public class PythonScripts {
    ...
    public static int getGenresCode(@NotNull String[] genres) {
        ...
        try {
            Process p = Runtime.getRuntime().exec("python scripts/genresCode.py " + filename);
    ...
    public static String[] getGPTMessages(Long id, String text) {
        try {
            ...
            output.close();
            try {
                Process p = Runtime.getRuntime().exec("python scripts/chatGPTScript.py " + "gpthistory/" + id.toString() + ".txt");
                BufferedReader reader = new BufferedReader(new InputStreamReader(p.getInputStream()));
                String temp;
